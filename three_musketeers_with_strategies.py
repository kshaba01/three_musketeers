...
def smart_musketeer():
  """this function aims to return a valid/legal musketeer move that ensures the 
  musketeers stay as far apart as possible. For each valid move, the cartesian distance between all
  musketeers is computed and the move with the highest distance will be selected. The distances are calculated as the 
  sum of all the distances between musketeers on the grid"""
  list=[]
  d = set(loc_musketeers())
  for move in all_possible_moves_for(M):
    mov = move[0]
    e = set()
    e.add(mov)
    M1 = adjacent_location(move[0], move[1])
    M2 = [x for x in set(d) - e][0] 
    M3 = [x for x in set(d) - e][1]
    z = abs(M1[0] - M2[0]) * abs(M1[1] - M2[1]) + abs(M1[0] - M3[0]) * abs(M1[1] - M3[1]) + abs(M2[0] - M3[0]) * abs(M2[1] - M3[1])
    list.append((z, move))
  return max(list)[1]
...
def loc_musketeers():
  ...
  for location in all_locations():
    if at(location) == M:
        loc_musketeers.append(location)
    
  return loc_musketeers
...
def choose_computer_move(who):
    ...
    #this uses the smart musketeer function which selects the move that maximises
    #the distance between the musketeers
    
    if who ==M:
      return smart_musketeer()
